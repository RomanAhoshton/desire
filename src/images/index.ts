import Logo from "./Logo.svg";
import facebook from "./facebook.svg";
import instagram from "./instagram.svg";
import pinterest from "./pinterest.svg";
import whatsapp from "./whatsapp.svg";
import youtube from "./youtube.svg";
import envelope from "./envelope.svg";
import BgBanner from "./bg-banner.jpg";
import BgSlider from "./bg-slider.svg";
import collection1 from "./collection1.jpg";
import collection2 from "./collection2.jpg";
import collection3 from "./collection3.jpg";
import collection4 from "./collection4.jpg";
import collection5 from "./collection5.jpg";
import collection6 from "./collection6.jpg";
import collection7 from "./collection7.jpg";
import collection8 from "./collection8.jpg";
import collection9 from "./collection9.jpg";
import collection10 from "./collection10.jpg";
import AestheticBg from "./Aesthetic-min.png";
import GrafficTool from "./graphic-tool.svg";
import Sofa from "./sofa.svg";
import Measuring from "./measuring-tape.svg";
import About from "./aboutPlaceholder.jpg";
import ChairImage from "./ChaireImage.png";
import GreenChair from "./Green_chair.png";
import Chair1 from "../images/Chair1.webp";
import Chair2 from "../images/Chair2.webp";
import Chair3 from "../images/Chair3.webp";
import Chair4 from "../images/Chair4.webp";
import Chair5 from "../images/Chair5.webp";
import Sofa1 from "../images/sofa1.webp";
import Sofa2 from "../images/sofa2.webp";
import Sofa3 from "../images/sofa3.webp";
import Sofa4 from "../images/sofa4.webp";
import Sofa5 from "../images/sofa5.webp";
import Table1 from "../images/Table1.webp";
import Table2 from "../images/Table2.webp";
import Table3 from "../images/Table3.webp";
import Table4 from "../images/Table4.webp";
import Table5 from "../images/Table5.webp";
import HomeSlider1 from "../images/HomeSlider1-min.jpg";
import HomeSlider2 from "../images/HomeSlider2-min.jpg";
import HomeSlider3 from "../images/HomeSlider3-min.jpg";
import HomeSlider4 from "../images/HomeSlider4-min.jpg";
import HomeSlider5 from "../images/HomeSlider5-min.jpg";
import HomeSlider6 from "../images/HomeSlider6-min.jpg";
import HomeSlider7 from "../images/HomeSlider7-min.jpg";
import HomeSlider8 from "../images/HomeSlider8-min.jpg";


export const SliderBg = BgSlider;
export const HomeBanner = BgBanner;
export const headerLogo = Logo;
export const footerFacebook = facebook;
export const footerInstagram = instagram;
export const footerPinterest = pinterest;
export const footerWhatsApp = whatsapp;
export const footerYoutube = youtube;
export const footerEnvelope = envelope;
export const BagAesthetic = AestheticBg;
export const AboutBanner = About;
export const chairAbout = ChairImage;
export const chairGreen = GreenChair;

export const collection = [
  {
    col: collection1,
  },
  {
    col: collection2,
  },
  {
    col: collection3,
  },
  {
    col: collection4,
  },
  {
    col: collection5,
  },
];
export const collectionAbout = [
  {
    col: collection6,
  },
  {
    col: collection7,
  },
  {
    col: collection8,
  },
  {
    col: collection9,
  },
  {
    col: collection10,
  },
];

export const HowWorksImages = {
  first: GrafficTool,
  second: Sofa,
  third: Measuring,
};

export const chairs = {
  one: Chair1,
  two: Chair2,
  three: Chair3,
  four: Chair4,
  five: Chair5,
};

export const sofa = {
  one: Sofa1,
  two: Sofa2,
  three: Sofa3,
  four: Sofa4,
  five: Sofa5,
};

export const table = {
  one: Table1,
  two: Table2,
  three: Table3,
  four: Table4,
  five: Table5,
};

export const HomeSlides = {
  one: HomeSlider1,
  two: HomeSlider2,
  three: HomeSlider3,
  four: HomeSlider4,
  five:HomeSlider5,
  six:HomeSlider6,
  seven:HomeSlider7,
  eight:HomeSlider8,
  
};
